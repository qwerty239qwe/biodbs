...
> Results generated by `benchmarks/` scripts (see `benchmarks/` directory).
> Speed numbers measure **library overhead only** — HTTP is mocked, so network latency is excluded.
> Reproducible with `uv run python benchmarks/speed_benchmark.py`.
